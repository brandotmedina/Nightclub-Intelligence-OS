    ...
    <>
      <main className={`min-h-screen bg-bg text-text ${event.vip_enabled ? "pb-8" : "pb-28"}`}>
        <div className="max-w-lg mx-auto px-4 pt-6 pb-6">
          ...
          {/* CTAs — two modes based on vip_enabled */}
          {event.vip_enabled ? (
            // VIP-ON: collapsible door tickets + VIP reservation side-by-side
            <EventCTAs
              eventId={event.id}
              price={event.price}
              isFree={isFree}
            ...
            // VIP-OFF: always-open ticket form (unchanged behaviour)
            <div
              id="tickets"
      ...
      {/* Sticky bottom CTA — VIP-OFF only; VIP-ON uses inline EventCTAs instead */}
      {!event.vip_enabled && (
